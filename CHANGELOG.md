...
- **Make the EA/PA methodology discoverable for ad-hoc work-shape questions** (issue #35). The first attempt placed a pointer in the plugin's root `CLAUDE.md` on the panel's assumption that plugin CLAUDE.md loads ambiently into Claude sessions. A real-session verification test showed that assumption was false: a fresh session tied to the profile still produced a generic PM audit and confirmed it had not read any plugin documentation. The pointer was moved to the SessionStart hook (`bin/session-hook.sh`), which is the channel already proven to reach sessions (its workplan-status output appears as an in-context reminder). Kept as a single line at the tail of the hook output, separated from state lines by a blank line to avoid register collision. Plugin CLAUDE.md retains the section as documentation for readers of the plugin source. `bin/test_start_prompts.py` adds a content-level regression assertion. Retest: cold-session audit that previously produced generic PM vocabulary now consults `docs/methodology.md` and applies principle-indexed reasoning.
