...
- **Make the EA/PA methodology discoverable for ad-hoc work-shape questions** (issue #35). Added a short section to the plugin's root `CLAUDE.md` that tells Claude to consult `docs/methodology.md` and `docs/triage-framework.md` before reasoning from first principles about prioritization / triage / carryover / deferral / budget / scoping / rhythm. No new injection mechanism: reuses the CLAUDE.md channel that Claude Code already loads ambiently per plugin. Pointer-based, not content-embedded — Claude fetches the doc on demand rather than reading duplicated principle text in every session. Chosen after a three-lens panel (plugin-ecosystem / cognitive-ergonomics / minimalist) converged on adding to an already-loaded surface rather than extending the SessionStart hook or scaffolding a per-profile CLAUDE.md.
